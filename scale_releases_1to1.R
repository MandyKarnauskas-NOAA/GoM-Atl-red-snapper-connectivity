...
par(mfrow=c(1,2))
sc2 <- scaled[which(scaled$V6==2004 & scaled$V7==7 & scaled$V8==17),]
x <- sc2$V5
...
sc3 <- unscaled[which(unscaled$V6==2004 & unscaled$V7==7 & unscaled$V8==17),]
x <- sc3$V5
...
sc2 <- scaled[which(scaled$V6==2004 & scaled$V7==3 & scaled$V8==1),]
x <- sc2$V5
pos <- c(0.005,  0.05, 0.1, 0.2, 0.5, 1, 2, 4, 5, 10, 50, 100, 200, 500, 1000)
a <- floor(min(x))
b <- max((x)-a)*1.03
...
datelist1 <- unique(paste(scaled$V7, ".", scaled$V8, sep=""))
datelist <- datelist[order(as.numeric(datelist1))]

windows()
par(mfrow=c(5,9), mex=0.4)

x <- scaled$V5
pos <- c(0.005,  0.05, 0.1, 0.2, 0.5, 1, 2, 4, 5, 10, 30, 100, 500, 1000)
a <- floor(min(x))
b <- max((x)-a)*1.03
cols <- c(rainbow(30, start=0.82, end=0.99), rainbow(70, start=0.01, end=0.17))[100:1]

for (i in 1:45)  {
sc2 <- scaled[which(scaled$V6==2004 & scaled$V7==strsplit(datelist[i], "_")[[1]][1] & scaled$V8==strsplit(datelist[i], "_")[[1]][2]),]
x1 <- sc2$V5
...
if(-pos[w]<min(x)) { xx <- seq(0, max(x), pos[w]); xx <- xx[xx>min(x)] } else {  xx <- c(seq(-pos[w], min(x), -pos[w]), seq(0, max(x), pos[w])) }
text(xloc[round((xx-a)/b*100+1)], y=23.2, xx, pos=2)
mtext(side=3, line=1.5, paste(strsplit(datelist[i], "_")[[1]][1], strsplit(datelist[i], "_")[[1]][2]), cex=1.3, font=2)
}
